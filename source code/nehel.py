...
class Face:
    def __init__(self):
        #size
        random_size_value = random.uniform(NEHEL_SIZE_RANDOMIZE[0], NEHEL_SIZE_RANDOMIZE[1])
        size = (int(NEHEL_SIZES[0] * random_size_value), int(NEHEL_SIZES[1] * random_size_value))
        # moving
        moving_direction, start_pos = self.define_spawn_pos(size)
        # sprite
        self.rect = pygame.Rect(start_pos[0], start_pos[1], size[0]//1.4, size[1]//1.4)
        self.images = [image.load("Assets/chris/chris.png", size=size, flip=moving_direction=="right")]
        self.current_frame = 0
        self.animation_timer = 0


    def define_spawn_pos(self, size): # define the start pos and moving vel of nehel
        vel = random.uniform(NEHEL_MOVE_SPEED["min"], NEHEL_MOVE_SPEED["max"])
    ...
    def kill(self, faces): # remove face of nehel from the list
        faces.remove(self)
        return 1

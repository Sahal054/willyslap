...
from nehel import Face
from nehelindevandi import Vandi
import cv2
import ui
...
class Game:
    def __init__(self, surface):
        ...
        self.sounds["daddy"] = pygame.mixer.Sound(f"Assets/Sounds/kozhitrim.wav")
        self.sounds["daddy"].set_volume(SOUNDS_VOLUME)
    ...
    def reset(self): # reset all the needed variables
        ...
        self.hand = Hand()
        self.nehel = []
        self.nehels_spawn_timer = 0
        self.score = 0
        self.game_start_time = time.time()


    def spawn_nehels(self):
        t = time.time()
        if t > self.nehels_spawn_timer:
            self.nehels_spawn_timer = t + NEHEL_SPAWN_TIME

            # increase the probability for object
            ...
            if random.randint(0, 100) < nb:
                self.nehel.append(Vandi())
            else:
                self.nehel.append(Face())

            # spawn a other nehel after the half of the game
    ...
    def update(self):
        ...
        if self.time_left > 0:
            ...
            self.score = self.hand.kill_nehel(self.nehel, self.score, self.sounds)
            for i in self.nehel:
                i.move()

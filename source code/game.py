...
from penalty_target import PenaltyTarget
import cv2
import ui
...
class Game:
    ...
    def spawn_targets(self):
        ...
        if t > self.targets_spawn_timer:
            self.targets_spawn_timer = t + TARGET_SPAWN_TIME

            # increase the probability for penalty target
            ...
            if random.randint(0, 100) < nb:
                self.targets.append(PenaltyTarget())
            else:
                self.targets.append(Target())

            # spawn another target after the half of the game
    ...
    def update(self):
        ...
        if self.time_left > 0:
            ...
            self.score = self.hand.kill_target(self.targets, self.score, self.sounds)
            for i in self.targets:
                i.move()

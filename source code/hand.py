...
class Hand:
    ...
    def on_target(self, targets): # return a list with all objects that collide with the hand hitbox
        return [i for i in targets if self.rect.colliderect(i.rect)]


    def kill_target(self, targets, score, sounds): # will kill the objects that collide with the hand when the left mouse button is pressed
